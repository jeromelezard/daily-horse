...
export default function Game() {
    ...
    const [phase, setPhase] = useState<"intro" | "guessing" | "revealing" | "finished">("intro");
    const [results, setResults] = useState<Result[]>([]);
    const [reveal, setReveal] = useState<{ guess: LatLng; truth: LatLng; score: number } | null>(null);
    ...
    function handleGuess(p: LatLng) {
        ...
        const truth = target.anywhere ? { lat: p.lat, lng: p.lng } : { lat: target.lat, lng: target.lng };
    ...
    const difficultyLabel = currentMultiplier === 1 ? "Easy" : currentMultiplier === 2 ? "Medium" : "Hard";
    ...
    return (
        ...
        >
            <Globe onGuess={handleGuess} reveal={reveal} interactive={phase === "guessing"} />

            <div className="absolute top-0 left-1/2 -translate-x-1/2 pointer-events-none" style={{ width: "352px", maxWidth: "100vw" }}>
                <div className="bg-cyan-400 text-black px-3 py-2 flex justify-between items-center text-xs font-medium pointer-events-auto">
                    ...
                    <div className="font-bold text-sm">BirthdayTap #{roundIndex + 1}</div>
                ...
                {phase !== "finished" && (
                    <div className="flex gap-2 px-2 pt-2">
                        <div className="border-2 border-cyan-400 bg-black/85 backdrop-blur rounded-md px-2.5 py-1 text-center pointer-events-auto self-start">
                            <div className="text-cyan-400 text-[10px] font-bold uppercase tracking-widest">Score</div>
                            <div className="text-white text-2xl font-bold tabular-nums leading-tight">{String(total).padStart(3, "0")}</div>
                        </div>
                        <div className="flex-1 flex flex-col rounded-md overflow-hidden pointer-events-auto">
                            ...
                            <div className="flex-1 flex items-center justify-center bg-zinc-800/90 backdrop-blur text-white px-3 py-2 text-center text-sm leading-snug">
                                Tap as close as you can to the {target.name.toLocaleLowerCase()} picture
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 flex justify-center">
                    <div className="bg-black/70 backdrop-blur px-4 py-3 sm:px-6 sm:py-4 rounded-lg flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-6 w-full max-w-md sm:w-auto sm:max-w-none">
                        <div className="flex justify-between sm:block gap-6">
                            <div>
                                <div className="text-[10px] sm:text-xs uppercase tracking-wide opacity-70">Distance</div>
                                <div className="text-lg sm:text-xl font-semibold">{Math.round(last.distanceKm).toLocaleString()} km</div>
                            </div>
                            <div className="sm:hidden">
                                <div className="text-[10px] uppercase tracking-wide opacity-70">Score</div>
                                <div className="text-lg font-semibold">{last.baseScore} / 100</div>
                            </div>
                        </div>
                        <div className="hidden sm:block">
                            <div className="text-xs uppercase tracking-wide opacity-70">Score</div>
                            <div className="text-xl font-semibold">{last.baseScore} / 100</div>
                        </div>
                        <button
                            onClick={handleNext}
            ...
            {phase === "intro" && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/70 p-4 z-20">
                    <div className="bg-zinc-900 p-6 sm:p-8 rounded-xl max-w-md w-full border border-white/10 text-center">
                        <p className="text-base sm:text-lg mb-6 leading-snug text-white">
                            I hope you enjoyed one of your birthday presents, but were you paying attention?
                        </p>
                        <button
            ...
            {phase === "finished" && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/70 p-4">
                    <div className="bg-zinc-900 p-6 sm:p-8 rounded-xl max-w-md w-full border border-white/10">
                        <h2 className="text-2xl sm:text-3xl font-bold mb-1 text-white">Done!</h2>
                        <p className="text-base text-white sm:text-lg opacity-80 mb-4">
                            You scored {total} / {MAX_TOTAL}
                        </p>
                        <ul className="space-y-2 mb-6">
                            {LOCATIONS.map((loc, i) => (
                                <li key={loc.name} className="flex justify-between border-b border-white/10 pb-1 text-sm">
                                    <span>{loc.name}</span>
